...
export default function MathMissionScreen({ count, difficulty, onSave, onBack }: Props) {
    ...
    const prevDiff = () => { if (diffIdx > 0) setLocalDiff(DIFFICULTY_LIST[diffIdx - 1]); };
    const nextDiff = () => { if (diffIdx < DIFFICULTY_LIST.length - 1) setLocalDiff(DIFFICULTY_LIST[diffIdx + 1]); };
    ...
    const handleBack = () => {
        if (isLoading) return;
        setIsLoading(true);
        showInterstitialAd('math_mission_screen', () => {
            setIsLoading(false);
            onBack();
        });
    };
    ...
    return (
        <View style={[S.root, { backgroundColor: colors.background, paddingTop: ins.top }]}>
            ...
            <View style={S.header}>
                ...
                <TouchableOpacity
                    style={[S.previewBtn, { backgroundColor: colors.primary + '18', borderColor: colors.primary + '44', opacity: isLoading ? 0.5 : 1 }]}
                    activeOpacity={isLoading ? 1 : 0.7}
                    disabled={isLoading}
                    onPress={handleSave}
                >
                    <Text style={[S.previewTxt, { fontSize: 15, fontWeight: '700', color: colors.primary }]}>
                        {t('save')}
                    </Text>
                </TouchableOpacity>
            </View>
            ...
            >
                <View style={[S.exampleCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                    <View style={S.flameRow}>
                        {[1, 2, 3, 4, 5].map(i => (
                            <Text key={i} style={[S.flame, { opacity: i <= meta.flames ? 1 : 0.25 }]}>🔥</Text>
                        ))}
                    </View>
                    ...
                    <View style={[S.dotRow, { marginBottom: 40 }]}>
                        {DIFFICULTY_LIST.map((d) => (
                            <View key={d} style={[S.dot, {
                                backgroundColor: d === localDiff ? colors.primary : colors.border,
                    ...
                    <View style={{ marginTop: 20 }}>
                        ...
                        <View style={S.controlRow}>
                            <TouchableOpacity
                                onPress={decCount} activeOpacity={0.7}
                                style={[S.circleBtn, {
                            ...
                            <TouchableOpacity
                                onPress={incCount} activeOpacity={0.7}
                                style={[S.circleBtn, {
                                    backgroundColor: colors.surface, borderColor: colors.border,
                    ...
                    <View style={{ marginTop: 30 }}>
                        ...
                        <View style={S.controlRow}>
                            <TouchableOpacity onPress={prevDiff} activeOpacity={0.7}
                            ...
                            <TouchableOpacity onPress={nextDiff} activeOpacity={0.7}
            ...
            <View style={S.stickyAdContainer}>
                <AdBanner screen="math_mission_screen" />
            </View>
...
const S = StyleSheet.create({
    ...
    previewBtn: { paddingHorizontal: 16, paddingVertical: 7, borderRadius: 20, borderWidth: 1 },
    previewTxt: {},
    exampleCard: { borderRadius: 20, borderWidth: 0.5, paddingVertical: 24, paddingHorizontal: 20, alignItems: 'center', marginTop: 16, marginBottom: 32 },
    ...
    sectionLabel: { fontSize: 12, fontWeight: '500', textAlign: 'center', marginBottom: 16 },
    controlRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 32, marginBottom: 32 },
    circleBtn: { width: 48, height: 48, borderRadius: 24, borderWidth: 1, alignItems: 'center', justifyContent: 'center' },
    ...
    saveWrap: { position: 'absolute', bottom: 0, left: 0, right: 0, paddingHorizontal: 20, paddingTop: 12 },
    saveBtn: { borderRadius: 16, paddingVertical: 18, alignItems: 'center', justifyContent: 'center' },
